...
STATE_DIR = Path("/var/lib/ups-cascade")
LIB_DIR = Path("/usr/local/lib/ups-cascade")
EVENTS_PATH = STATE_DIR / "events.jsonl"
STATE_PATH = STATE_DIR / "state.json"
KUBECONFIG = STATE_DIR / "kubeconfig"
TALOSCONFIG = STATE_DIR / "talosconfig"
NAS_SSH_KEY = STATE_DIR / ".ssh" / "id_ed25519"
NAS_KNOWN_HOSTS = STATE_DIR / ".ssh" / "known_hosts"
TALOS_SCRIPT = LIB_DIR / "talos-shutdown.sh"
CEPH_NAMESPACE = "rook-ceph"

# Always name the credentials explicitly. Relying on the environment reaching the
# subprocess cost a whole Phase 7 run: talosctl fell back to $HOME/.talos/config,
# which systemd services do not have, and every shutdown call failed silently.
KUBECTL = ["kubectl", "--kubeconfig", str(KUBECONFIG)]
TALOSCTL = ["talosctl", "--talosconfig", str(TALOSCONFIG)]
...
class Config:
    def __init__(self) -> None:
        ...
        self.fsd_charge = env_int("UPS_CASCADE_FSD_CHARGE", 20)
...
class Cascade:
    def __init__(self, cfg: Config, log: EventLog, state: State) -> None:
        ...
        self.nodes_down_escalated = False
    ...
    def tick_nodes_down(self, snap: dict, ob: bool) -> None:
        ...
        # The script reports only that it ran the commands. If the nodes are still
        # answering, the shutdown did not take - retry from here, where the per-node
        # exit status is logged, before writing them off.
        if not self.nodes_down_escalated:
            self.nodes_down_escalated = True
            self.log.event(
                "talos_nodes_down_timeout", snap, phase=NODES_DOWN,
                pending=",".join(pending), action="escalate",
            )
            self.force_shutdown_nodes(snap)
            self.set_phase(NODES_DOWN)
            return

        self.log.event("talos_nodes_down_giving_up", snap, phase=NAS, pending=",".join(pending))
        self.set_phase(NAS)
    ...
    def nas_ssh_argv(self) -> list[str]:
        return [
            ...
            "-o", f"UserKnownHostsFile={NAS_KNOWN_HOSTS}",
            "-o", "ConnectTimeout=10",
    ...
    def tick_handoff(self, snap: dict, ob: bool) -> None:
        ...
        # Normally the Pi is shut down moments from here and this state dies with
        # it. If mains comes back first the cascade is over but nothing powered
        # off, so return to IDLE rather than wedging until someone restarts us.
        if not ob and self.ol_since is not None:
            if (time.monotonic() - self.ol_since) >= self.cfg.ol_stable:
                self.log.event("cascade_ended", snap, phase=IDLE, reason="mains_restored_after_handoff")
                self.reset_run()
                self.set_phase(IDLE)
            return

        # Own the timing of the final shutdown rather than waiting for upsmon's
        # LB. On this UPS the LB flag comes only from the device's HID status bits
        # and trips at runtime <= 300 - a signal measured as quantised to 60s
        # steps and non-monotonic, which flickered across its own threshold during
        # a real run. battery.charge is monotonic and well behaved, so gate on it.
        # upsmon still does all the actual work: FSD, POWERDOWNFLAG, SHUTDOWNCMD
        # and killpower. We only choose when. This arms strictly after the servers
        # are down, so it cannot fire mid-drain.
        if self.fsd_sent:
            return
        charge = telemetry(snap).get("charge")
        if charge is None or charge > self.cfg.fsd_charge:
            return

        self.fsd_sent = True
        self.log.event("fsd_triggered", snap, phase=HANDOFF, floor=self.cfg.fsd_charge)
        rc, out = run(["upsmon", "-c", "fsd"], timeout=30)
        self.log.event("fsd_issued", snap, phase=HANDOFF, rc=rc, output=out[:200])


def ceph_health(cfg: Config) -> tuple[bool, str]:
    """Read health from the CephCluster CR.

    Deliberately not `kubectl exec` into rook-ceph-tools: the toolbox does not
    carry the rook_cluster label, so the drain deletes it, and it cannot come
    back until the nodes are uncordoned. Anything that execs into it therefore
    deadlocks against the very uncordon it is gating.
    """
    rc, out = run(
        KUBECTL + ["-n", CEPH_NAMESPACE, "get", "cephcluster", "-o", "json"],
        env=cfg.kube_env(),
        timeout=60,
    )
    if rc != 0:
        return False, f"cephcluster unreadable (rc={rc})"
    try:
        payload = json.loads(out[out.index("{"):out.rindex("}") + 1])
        items = payload.get("items", [])
        health = items[0]["status"]["ceph"]["health"] if items else ""
    except (ValueError, KeyError, IndexError):
        return False, "unparseable cephcluster status"
    if health == "HEALTH_OK":
        return True, health
    return False, health or "no health reported"
...
def cmd_recover(cfg: Config, log: EventLog, state: State) -> int:
    ...
    # Nodes Ready is the ONLY precondition. After a power cycle nothing can
    # schedule until the cordon is lifted - including every Ceph daemon - so any
    # gate on Ceph being healthy first can never be satisfied.
    last = ""
    ...
    if not cascade.uncordon(ups.read()):
    ...
    # Informational only, after the marker is cleared: report when storage is
    # actually serving again so the log shows how long the cold start took.
    last = ""
    while time.monotonic() < deadline:
        ok, detail = ceph_health(cfg)
        if ok:
            log.event("recover_ceph_healthy", ups.read(), phase=IDLE, detail=detail)
            return 0
        if detail != last:
            log.event("recover_ceph_waiting", ups.read(), phase=IDLE, detail=detail)
            last = detail
        time.sleep(30)

    log.event("recover_ceph_timeout", ups.read(), phase=IDLE, detail=last)
    return 0
