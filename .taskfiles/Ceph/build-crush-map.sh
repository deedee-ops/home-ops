...
yq -o json '.rook-ceph-cluster.cephClusterSpec.storage' < "talos/$1/values.bootstrap.yaml" > /tmp/ceph-config.json
...
DISKS="$(jq -r '.nodes[].devices[].name' < /tmp/ceph-config.json)"
