...
for osd in $(ceph osd ls); do ceph osd metadata "${osd}" | jq -r '(.bluestore_bdev_dev_node + ":" + .hostname + " " + (.id|tostring) + " osd." + (.id|tostring))'; done | sed -f <(ceph osd tree | grep osd\. | awk '{ print "s@" $4 "@" $3 "@g" }') > /tmp/ceph-osd
