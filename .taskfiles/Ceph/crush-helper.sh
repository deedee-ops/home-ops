...
jq -r '.nodes[] | . as $node | .devices[] | ("ceph osd crush add-bucket " + $node.name + "-" + .config.crushRoot + " host")' < "$1"
jq -r '.nodes[] | . as $node | .devices[] | ("ceph osd crush move " + $node.name + "-" + .config.crushRoot + " root=" + .config.crushRoot)' < "$1"
...
jq -r '.nodes[] | . as $node | .devices[] | ("ceph osd crush set osd." + .name + ":" + $node.name + " WEIGHT root=" + .config.crushRoot + " host=" + $node.name + "-" + .config.crushRoot)' < "$1" | sed -f <(awk '{ print "s@" $1 "@" $2 "@g" }' /tmp/disk-map) | sed -f <(awk '{ print "s@" $1 " WEIGHT@" $2 " " $3 "@g"}' /tmp/ceph-osd)
